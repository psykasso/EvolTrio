package gr.evomusic.evoltrio.core;

import gr.evomusic.evoltrio.conf.CliParametersParser;
import gr.evomusic.evoltrio.midi.MusicConfiguration;
import gr.evomusic.evoltrio.midi.SongBuilder;
...
public class Evolution {
	...
	public void setup() {
		...
		try {
			for (int i = 0; i < musicConf.getPhraseNotes() * 2; i += 2) {
				sampleGenes[i] = new IntegerGene(evolConf, musicConf.getMaxIntervalJump() * (-1),
						musicConf.getMaxIntervalJump());
				sampleGenes[i + 1] = new IntegerGene(evolConf, musicConf.getMaxDurationJump()
						* (-1), musicConf.getMaxDurationJump());
			}
	...
	/**
	 * @param evolConf the evolConf to set
	 */
